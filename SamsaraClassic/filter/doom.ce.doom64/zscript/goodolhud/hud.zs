class GoodOlHUDStatusBar : BaseStatusBar
{
    ...
    protected void GOHDrawFullScreenStuff ()
    {
        ...
        let canshowberserk = CVar.FindCVar("goh_showberserk").GetInt() && (Powerup(CPlayer.mo.FindInventory("PowerStrength")) || Powerup(CPlayer.mo.FindInventory("D64PowerStrength")));
    ...
    protected virtual void GOHDrawMiscItems(int coordbasex, int coordbasey)
    {
        ...
        if (activemiscitems > 0)
        {
            for (checkedmiscitems = 0; checkedmiscitems < maxmiscitems; checkedmiscitems += 4)
            {
                ...
                bool ispowerup = false, isoutcastartifact = false;
                ...
                if (TexMan.CheckForTexture(currentmiscitemicon).IsValid())
                {
                    ...
                    if (showcooldowntimer)
                    {
                        if (ispowerup)
                        {
                            ...
                            if (currentmiscitempoweruptimer) { DrawString(GOHmHUDFont, "\c" .. currentmiscitemcolor .. FormatNumber((currentmiscitempoweruptimer.EffectTics / TICRATE) + (CVar.FindCVar("goh_effecttimerdisplaymode").GetBool() ? 1 : 0), 1, 4), (coordbasex - (isoutcastartifact ? 23 : 16), (coordbasey + coordnudge.Y) - (isoutcastartifact ? 6 : 0)), DI_SCREEN_RIGHT_TOP|DI_TEXT_ALIGN_RIGHT, Font.CR_UNTRANSLATED, GOHHideOnDeath(1., PlayerDeadTimerCheckMiscItems)); }
                        } else {
                            let currentmiscitemtimer = CPlayer.mo.FindInventory(MiscItemDefinitions[checkedmiscitems + 2]);

                            if (currentmiscitemtimer) { DrawString(GOHmHUDFont, "\c" .. currentmiscitemcolor .. FormatNumber(GetAmount(MiscItemDefinitions[checkedmiscitems + 2]) - (CVar.FindCVar("goh_effecttimerdisplaymode").GetBool() ? 0 : 1), 1, 4), (coordbasex - (isoutcastartifact ? 23 : 16), (coordbasey + coordnudge.Y) - (isoutcastartifact ? 6 : 0)), DI_SCREEN_RIGHT_TOP|DI_TEXT_ALIGN_RIGHT, Font.CR_UNTRANSLATED, GOHHideOnDeath(1., PlayerDeadTimerCheckMiscItems)); }
                        }
                    }
