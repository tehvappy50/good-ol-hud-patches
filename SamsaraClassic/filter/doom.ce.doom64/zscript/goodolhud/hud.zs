class GoodOlHUDStatusBar : BaseStatusBar
{
    ...
    protected void GOHDrawFullScreenStuff ()
    {
        ...
        let canshowberserk = CVar.FindCVar("goh_showberserk").GetBool() && (Powerup(CPlayer.mo.FindInventory("PowerStrength")) || Powerup(CPlayer.mo.FindInventory("D64PowerStrength")));
        ...
        // Cooldown timers
        GOHDrawCooldownTimers((-274 - 1) + coordnudge.X, (-4 + 3) - (showingcurrentitem ? 37 + (CPlayer.mo.InvSel.Amount > 1 || canalwaysshowinvcounter ? 16 : 0) : 0) + coordnudge.Y, (-406 + 4) + coordnudge.X, -17 + coordnudge.Y);
    ...
    protected virtual void GOHDrawWeaponName(int coordbasex, int coordbasey)
    {
        ...
        if (CPlayer.ReadyWeapon)
        {
            ...
            DrawString(GOHmHUDFont,
                       "\c" .. colorschemetext ..
                       (usingspectralweapon ? StringTable.Localize("$GOODOLHUD_WEAPON_SAMSARA_SPECTRAL") .. " " : "") .. CPlayer.ReadyWeapon.GetTag() ..
    ...
    protected virtual void GOHDrawWeaponBar(int coordbasex, int coordbasey)
    {
        ...
        DrawString(GOHmHUDFont, hasslot4 || usingslot4 ? "\c" .. (usingslot4 ? colorschemeactivetext : colorschemetext) .. FormatNumber(4, 1, 1) : "", (coordbasex + 30, coordbasey), weaponbarflags);
        DrawString(GOHmHUDFont, hasslot5 || usingslot5 ? "\c" .. (usingslot5 ? colorschemeactivetext : colorschemetext) .. FormatNumber(5, 1, 1) : "", (coordbasex + 40, coordbasey), weaponbarflags);
        DrawString(GOHmHUDFont, hasslot6 || usingslot6 ? "\c" .. (usingslot6 ? colorschemeactivetext : colorschemetext) .. FormatNumber(6, 1, 1) : "", (coordbasex + 50, coordbasey), weaponbarflags);
        DrawString(GOHmHUDFont, hasslot7 || usingslot7 ? "\c" .. (usingslot7 ? colorschemeactivetext : colorschemetext) .. FormatNumber(7, 1, 1) : "", (coordbasex + 60, coordbasey), weaponbarflags);
    }
                ...
                if (TexMan.CheckForTexture(currentmiscitemicon).IsValid())
                {
                    ...
                    if (showcooldowntimer)
                    {
                        if (ispowerup)
                        {
                            ...
                            if (currentmiscitempoweruptimer) { DrawString(GOHmHUDFont, "\c" .. currentmiscitemcolor .. FormatNumber(currentmiscitempoweruptimer.EffectTics / TICRATE, 1, 4), (coordbasex - (isoutcastartifact ? 23 : 16), (coordbasey + coordnudge.Y) - (isoutcastartifact ? 6 : 0)), DI_SCREEN_RIGHT_TOP|DI_TEXT_ALIGN_RIGHT); }
                        } else {
                            let currentmiscitemtimer = CPlayer.mo.FindInventory(MiscItemDefinitions[checkedmiscitems + 2]);

                            if (currentmiscitemtimer) { DrawString(GOHmHUDFont, "\c" .. currentmiscitemcolor .. FormatNumber(GetAmount(MiscItemDefinitions[checkedmiscitems + 2]) - 1, 1, 4), (coordbasex - (isoutcastartifact ? 23 : 16), (coordbasey + coordnudge.Y) - (isoutcastartifact ? 6 : 0)), DI_SCREEN_RIGHT_TOP|DI_TEXT_ALIGN_RIGHT); }
                        }
                    }
