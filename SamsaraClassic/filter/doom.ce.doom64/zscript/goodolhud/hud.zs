                ...
                if (TexMan.CheckForTexture(currentmiscitemicon).IsValid())
                {
                    ...
                    if (showcooldowntimer)
                    {
                        if (ispowerup)
                        {
                            ...
                            if (currentmiscitempoweruptimer) { DrawString(GOHmHUDFont, "\c" .. currentmiscitemcolor .. FormatNumber(currentmiscitempoweruptimer.EffectTics / TICRATE, 1, 4), (coordbasex - (isoutcastartifact ? 23 : 16), (coordbasey + coordnudge.Y) - (isoutcastartifact ? 6 : 0)), DI_SCREEN_RIGHT_TOP|DI_TEXT_ALIGN_RIGHT, Font.CR_UNTRANSLATED, GOHHideOnDeath(1., PlayerDeadTimerCheckMiscItems)); }
                        } else {
                            let currentmiscitemtimer = CPlayer.mo.FindInventory(MiscItemDefinitions[checkedmiscitems + 2]);

                            if (currentmiscitemtimer) { DrawString(GOHmHUDFont, "\c" .. currentmiscitemcolor .. FormatNumber(GetAmount(MiscItemDefinitions[checkedmiscitems + 2]) - 1, 1, 4), (coordbasex - (isoutcastartifact ? 23 : 16), (coordbasey + coordnudge.Y) - (isoutcastartifact ? 6 : 0)), DI_SCREEN_RIGHT_TOP|DI_TEXT_ALIGN_RIGHT, Font.CR_UNTRANSLATED, GOHHideOnDeath(1., PlayerDeadTimerCheckMiscItems)); }
                        }
                    }
